...
class CamData(object):
            ...
            if self._check_cache_for_current_subset():
                ...
                logger.info(
                    "{}: cache file {} not valid, rebuilding cache...".format(
                        CamData.__name__, cache_path))
        # build subset cache in CamData.data_dict and cache to storage
        self._build_cache_for_current_subset()
        logger.info("{}: current cache file: {} ".format(
            CamData.__name__, self.cache_path))
        logger.info(
    ...
    def _build_cache_for_current_subset(self):
        ...
        subset = self.subset
        for s in subset:
            logger.info("{}: start loading {}".format(CamData.__name__, s))
