...
from ..datasets import CamData
from ..utils.metrics import center_error, normalized_center_error, rect_iou
from .otb import ExperimentOTB


class ExperimentCamData(ExperimentOTB):
    r"""Experiment pipeline and evaluation toolkit for CamData dataset.
    
    Args:
        root_dir (string): Root directory of CamData dataset.
        subset (string, optional): Specify ``train`` or ``test``
            subset of CamData.  Default is ``test``.
