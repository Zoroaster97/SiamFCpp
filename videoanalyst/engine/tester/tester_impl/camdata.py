...
from videoanalyst.evaluation.got_benchmark.experiments import ExperimentCamData
...
class CamDataTester(TesterBase):
    r"""CamData tester
    ...
    extra_hyper_params = dict(
        device_num=1,
        data_root="datasets/CamData",
        subsets=["test"],  # (train|test|train_test)
    )

    def __init__(self, *args, **kwargs):
        super(CamDataTester, self).__init__(*args, **kwargs)
...
CamDataTester.default_hyper_params = copy.deepcopy(
    CamDataTester.default_hyper_params)
CamDataTester.default_hyper_params.update(CamDataTester.extra_hyper_params)
